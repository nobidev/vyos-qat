...
/* Copyright(c) 2015 - 2017, 2019 - 2021 Intel Corporation */
			       ...
			       struct qat_crypto_request *qat_req)
{
	...
	if ((n + assoc_n) > QAT_MAX_PRE_ALLOCATED_BUFFS) {
...
static void qat_alg_update_iv_cbc_mode(struct qat_crypto_request *qat_req)
{
	...
	int offset = areq->nbytes - AES_BLOCK_SIZE;
	struct scatterlist *sgl;
				     ...
				     unsigned int keylen)
{
	...
	ctx->mode = ICP_QAT_HW_CIPHER_CBC_MODE;
...
int qat_algs_register(void)
{
	...
	if (++active_devs == 1) {
		...
		if (ret)
			goto unlock;
#ifdef QAT_LEGACY_ALGORITHMS
	for (i = 0; i < ARRAY_SIZE(qat_legacy_algs); i++)
		qat_legacy_algs[i].cra_flags =
			(qat_legacy_algs[i].cra_type == &crypto_aead_type) ?
			CRYPTO_ALG_TYPE_AEAD | CRYPTO_ALG_ASYNC :
			CRYPTO_ALG_TYPE_ABLKCIPHER | CRYPTO_ALG_ASYNC;
