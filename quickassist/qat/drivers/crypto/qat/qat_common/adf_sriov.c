...
static int adf_get_arbitrary_numvfs(struct adf_accel_dev *accel_dev,
				    const int numvfs)
{
...
#ifdef QAT_DBG
		if (qat_dbg_init_instance_vf(vf_info))
			return -EFAULT;
#endif
	}
...
static int adf_sriov_enable(struct adf_accel_dev *accel_dev, const int numvfs)
{
	...
	/* Configure capabilities and accel units for devices that support
	 * accel unit configuration before enabling vf through sys file system
	 * when PF is in down state.
	 */
	if (!adf_dev_started(accel_dev) && !adf_devmgr_in_reset(accel_dev) &&
	    GET_HW_DATA(accel_dev)->configure_accel_units) {
		if (GET_HW_DATA(accel_dev)->get_accel_cap)
			GET_HW_DATA(accel_dev)->accel_capabilities_mask =
			GET_HW_DATA(accel_dev)->get_accel_cap(accel_dev);
		if (GET_HW_DATA(accel_dev)->configure_accel_units(accel_dev))
			goto err_del_cfg;
	}
...
int adf_sriov_configure(struct pci_dev *pdev, int numvfs)
{
	...
	if (accel_dev->hw_device->get_arbitrary_numvfs)
		numvfs = accel_dev->hw_device->get_arbitrary_numvfs(accel_dev,
								    numvfs);
	else
		numvfs = adf_get_arbitrary_numvfs(accel_dev, numvfs);
