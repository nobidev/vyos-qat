...
#ifdef QAT_DBG
	if (qat_dbg_sysfs_cfg_add(accel_dev))
		goto err;
#endif
...
int adf_cfg_dev_add_ext(struct adf_accel_dev *accel_dev)
{
	int ret = 0;
	...
	if (!accel_dev->is_vf &&
		...
		ret = adf_cnvnr_freq_counters_add(accel_dev);
...
void adf_cfg_dev_remove_ext(struct adf_accel_dev *accel_dev)
{
	if (!accel_dev->is_vf &&
	    (GET_HW_DATA(accel_dev)->accel_capabilities_mask &
	     ADF_ACCEL_CAPABILITIES_COMPRESSION))
				...
				enum adf_cfg_val_type type)
{
	 ...
	 */
	if (adf_cfg_get_param_value(accel_dev, section_name,
				    key, temp_val) == 0) {
...
static int adf_cfg_section_save_all(struct adf_accel_dev *accel_dev,
				    struct adf_cfg_depot_list *cfg_depot_list)
{
	struct adf_cfg_section *ptr_sec, *iter_sec;
