...
/* Copyright(c) 2015 - 2018, 2020 - 2021 Intel Corporation */
...
static ssize_t sku_show(struct adf_uio_control_accel *accel, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n",
			accel->accel_dev->accel_pci_dev.sku);
}
...
static struct accel_sysfs_entry accel_sku = __ATTR_RO(sku);
...
int adf_uio_sysfs_create(struct adf_accel_dev *accel_dev)
{
	...
	if (!nb_bundles)
		/* Zero user bundles found. Skipping uio sysfs creation */
		return 0;
