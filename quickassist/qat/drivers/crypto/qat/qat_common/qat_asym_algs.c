...
/* Copyright(c) 2015 - 2019, 2021 Intel Corporation */
...
static int qat_dh_set_secret(struct crypto_kpp *tfm, QAT_PKE_CONST void *buf,
			     unsigned int len)
{
	struct qat_dh_ctx *ctx = kpp_tfm_ctx(tfm);
...
static QAT_PKE_MAX_SZ_SIGN int qat_dh_max_size(struct crypto_kpp *tfm)
{
	struct qat_dh_ctx *ctx = kpp_tfm_ctx(tfm);
...
static void qat_rsa_setkey_crt(struct qat_rsa_ctx *ctx, struct rsa_key *rsa_key)
{
	...
	/*
	 *  Detect and discard invalid rsa key second form parameters from
	 *  kernel 4.13 .. 4.15
	 */
	if (rsa_key->p_sz == 1 && *rsa_key->p == 0x30 &&
	    rsa_key->q_sz == 1 && *rsa_key->q == 0x30 &&
	    rsa_key->dp_sz == 1 && *rsa_key->dp == 0x30 &&
	    rsa_key->dq_sz == 1 && *rsa_key->dq == 0x30 &&
	    rsa_key->qinv_sz == 1 && *rsa_key->qinv == 0x30) {
		pr_info_once("QAT: Invalid rsa key -- ignoring second form\n");
		goto err;
	}
