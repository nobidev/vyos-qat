...
static int adf_iov_compat_checker_ring_svc_map(struct adf_accel_dev *accel_dev,
					       u8 vf_compat_ver)
{
	...
	if (!cm) {
		dev_err(&GET_DEV(accel_dev),
			"compatibility manager not initialized.\n");
		return -ENOMEM;
	}

	/* On PF check for VF compatibility and ring to service mapping */
	if (vf_compat_ver < ADF_PFVF_COMPATIBILITY_RING_TO_SVC_MAP &&
	    ring_to_svc_map != ADF_DEFAULT_RING_TO_SRV_MAP) {
		compatible = ADF_PF2VF_VF_INCOMPATIBLE;
	}
			    ...
			    bool is_notification)
{
	...
	if (is_notification && (val & ~int_bit) != msg) {
		...
		dev_err(&GET_DEV(accel_dev), "Collision on notification\n");
	...
	if ((val & local_in_use_mask) == local_in_use_pattern)
		ADF_CSR_WR(pmisc_bar_addr, pf2vf_offset,
...
static int adf_iov_put(struct adf_accel_dev *accel_dev, u32 msg, u8 vf_nr,
		       bool is_notification)
{
...
void adf_vf2pf_req_hndl(struct adf_accel_vf_info *vf_info)
{
	...
	case ADF_VF2PF_MSGTYPE_GET_SMALL_BLOCK_REQ:
		...
		case ADF_VF2PF_MSGTYPE_GET_LARGE_BLOCK_REQ:
			...
			break;
		case ADF_VF2PF_MSGTYPE_GET_MEDIUM_BLOCK_REQ:
			byte_num = ((msg & ADF_VF2PF_MEDIUM_BLOCK_BYTE_NUM_MASK)
				    >> ADF_VF2PF_MEDIUM_BLOCK_BYTE_NUM_SHIFT);
			msg_type = ((msg & ADF_VF2PF_MEDIUM_BLOCK_REQ_TYPE_MASK)
				    >> ADF_VF2PF_BLOCK_REQ_TYPE_SHIFT);
			msg_type += ADF_VF2PF_MIN_MEDIUM_MESSAGE_TYPE;
			break;
		case ADF_VF2PF_MSGTYPE_GET_SMALL_BLOCK_REQ:
			byte_num = ((msg & ADF_VF2PF_SMALL_BLOCK_BYTE_NUM_MASK)
				    >> ADF_VF2PF_SMALL_BLOCK_BYTE_NUM_SHIFT);
			msg_type = ((msg & ADF_VF2PF_SMALL_BLOCK_REQ_TYPE_MASK)
				    >> ADF_VF2PF_BLOCK_REQ_TYPE_SHIFT);
			msg_type += ADF_VF2PF_MIN_SMALL_MESSAGE_TYPE;
			break;
		default:
			dev_err(&GET_DEV(accel_dev),
				"Unknown block message from VF%d (0x%x);\n",
				vf_nr + 1, msg);
			goto out;
		}

		if (msg >> ADF_VF2PF_BLOCK_REQ_CRC_SHIFT) {
			res = adf_iov_block_get_crc(accel_dev, msg_type,
						    byte_num, &data,
						    vf_info->compat_ver);
			if (res)
			...
			res = adf_iov_block_get_byte(accel_dev, msg_type,
						     byte_num, &data,
						     vf_info->compat_ver);
			if (res)
				resp_type = ADF_PF2VF_BLOCK_RESP_TYPE_ERROR;
	...
	case ADF_VF2PF_MSGTYPE_NOTIFY:
		{
		switch ((msg & ADF_VF2PF_MSGDATA_MASK) >>
			ADF_VF2PF_MSGDATA_SHIFT) {
...
void adf_pf2vf_notify_fatal_error(struct adf_accel_dev *accel_dev)
{
	...
	for (i = 0, vf = accel_dev->pf.vf_info; i < num_vfs; i++, vf++) {
		...
		if (!vf->qatd_fake_dev)
			continue;
		if (dbg_map_vf < 0) {
			/* Dump processes memory map only once for all VFs */
			qat_dbg_phy_map_store(vf->qatd_fake_dev);
			dbg_map_vf = i;
		} else {
			/* Copy processes memory map from first VF */
			qat_dbg_phy_map_copy(vf->qatd_fake_dev,
					     accel_dev->pf.vf_info[dbg_map_vf]
					     .qatd_fake_dev);
		}

		adf_error_notifier((uintptr_t)vf->qatd_fake_dev);
#endif
	}
}

static int adf_iov_register_compat_checker(struct adf_accel_dev *accel_dev,
...
static int adf_iov_unregister_compat_checker(struct adf_accel_dev *accel_dev,
					     const adf_iov_compat_checker_t cc)
{
	struct adf_accel_compat_manager *cm = accel_dev->cm;
	int num = 0;
...
static int adf_iov_init_compat_manager(struct adf_accel_dev *accel_dev,
				       struct adf_accel_compat_manager **cm)
{
	...
	if (adf_iov_register_compat_checker
				(accel_dev,
				 adf_iov_compat_checker_ring_svc_map)) {
		dev_err(&GET_DEV(accel_dev),
			"compatibility checker register fail\n");
		return -EFAULT;
	}
	return 0;
}

static int adf_iov_shutdown_compat_manager(struct adf_accel_dev *accel_dev,
					   struct adf_accel_compat_manager **cm)
{
	if (*cm) {
		if (adf_iov_unregister_compat_checker
					(accel_dev,
					 adf_iov_compat_checker_ring_svc_map)) {
			dev_err(&GET_DEV(accel_dev),
				"compatibility checker unregister fail\n");
			return -EFAULT;
		}
		kfree(*cm);
		*cm = NULL;
