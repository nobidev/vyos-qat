...
/* Copyright(c) 2013 - 2017, 2019 - 2021, 2023 Intel Corporation */
...
struct icp_qat_css_fw_size {
	unsigned int css_fwsk_modulus_len;
	unsigned int css_fwsk_pad_len;
	unsigned int css_signature_len;
	unsigned int css_fwsk_exponent_len;
	unsigned int css_fwsk_pub_len;
	unsigned int css_ae_simg_len;
	unsigned int ae_img_offset;
};
...
static void qat_uclo_check_css_fw_size(struct icp_qat_fw_loader_handle *handle,
				       struct icp_qat_css_fw_size *css_fw_size)
{
	struct icp_qat_css_fw_size size = { 0 };

	switch (handle->pci_dev->device) {
	default:
		size.css_fwsk_modulus_len = ICP_QAT_CSS_2K_FWSK_MODULUS_LEN;
		size.css_fwsk_pad_len = ICP_QAT_CSS_2K_FWSK_PAD_LEN;
		size.css_signature_len = ICP_QAT_CSS_2K_SIGNATURE_LEN;
		size.css_fwsk_exponent_len = ICP_QAT_CSS_2K_FWSK_EXPONENT_LEN;
		size.css_fwsk_pub_len = ICP_QAT_CSS_2K_FWSK_PUB_LEN;
		size.css_ae_simg_len = ICP_QAT_CSS_2K_AE_SIMG_LEN;
		size.ae_img_offset = ICP_QAT_AE_2K_IMG_OFFSET;
	}

	/* Return CSS firmware size by pointer */
	*css_fw_size = size;
}

static int qat_uclo_wr_sram_by_words(struct icp_qat_fw_loader_handle *handle,
				...
				struct icp_qat_uclo_encapme *image)
{
	...
	fill_data = kcalloc(obj_handle->ustore_phy_size * 2, sizeof(uint64_t),
			    GFP_KERNEL);
	if (!fill_data)
		return -ENOMEM;
	...
	for_each_set_bit(ae, &ae_mask, handle->hal_handle->ae_max_num) {
		...
		if (obj_handle->ae_data[ae].shareable_ustore) {
			...
			qat_hal_wr_coalesce_uwords(handle, (unsigned char)ae,
						   patt_pos,
...
qat_uclo_get_dev_type(struct icp_qat_fw_loader_handle *handle)
{
	...
	case ADF_D15XX_PCI_DEVICE_ID:
		return ICP_QAT_AC_D15XX_DEV_TYPE;
			     ...
			     struct icp_qat_uof_image *uof_image)
{
	...
	unsigned short mode = uof_image->ae_mode;

	if (qat_hal_set_ae_ctx_mode(handle, ae,
				    (char)ICP_QAT_CTX_MODE(mode))) {
		pr_err("QAT: qat_hal_set_ae_ctx_mode error\n");
		return -EFAULT;
	}
	qat_hal_set_ae_scs_mode(handle, ae,
				(char)ICP_QAT_SHARED_USTORE_MODE(mode));
	nn_mode = ICP_QAT_NN_MODE(mode);
	...
	if (qat_hal_set_ae_lm_mode(handle, ae, ICP_LMEM0,
				   (char)ICP_QAT_LOC_MEM0_MODE(mode))) {
		pr_err("QAT: qat_hal_set_ae_lm_mode LMEM0 error\n");
		return -EFAULT;
	}
	if (qat_hal_set_ae_lm_mode(handle, ae, ICP_LMEM1,
				   (char)ICP_QAT_LOC_MEM1_MODE(mode))) {
		pr_err("QAT: qat_hal_set_ae_lm_mode LMEM1 error\n");
		return -EFAULT;
	}
	if (obj_handle->prod_type == ICP_QAT_AC_C4XXX_DEV_TYPE) {
		if (qat_hal_set_ae_lm_mode(handle, ae, ICP_LMEM2,
					   (char)ICP_QAT_LOC_MEM2_MODE(mode
									))) {
			pr_err("QAT: qat_hal_set_ae_lm_mode LMEM2 error\n");
			return -EFAULT;
		}
		if (qat_hal_set_ae_lm_mode(handle, ae, ICP_LMEM3,
					   (char)ICP_QAT_LOC_MEM3_MODE(mode
									))) {
			pr_err("QAT: qat_hal_set_ae_lm_mode LMEM3 error\n");
			return -EFAULT;
		}
		qat_hal_set_ae_tindex_mode(handle, ae,
					   (char)ICP_QAT_LOC_TINDEX_MODE(mode
									 ));
	}
	return 0;
}
...
static int qat_uclo_parse_uof_obj(struct icp_qat_fw_loader_handle *handle)
{
	...
	if (obj_handle->prod_type == ICP_QAT_AC_C4XXX_DEV_TYPE) {
		obj_handle->ustore_phy_size = 0x2000;
	} else {
		obj_handle->ustore_phy_size = 0x4000;
	}
			      ...
			      struct icp_qat_suof_chunk_hdr *suof_chunk_hdr)
{
	...
	qat_uclo_check_css_fw_size(handle, &css_fw_size);
	...
	suof_img_hdr->css_signature = suof_img_hdr->css_key +
				      css_fw_size.css_fwsk_modulus_len +
				      css_fw_size.css_fwsk_exponent_len;

	suof_img_hdr->css_simg = suof_img_hdr->css_signature +
				 css_fw_size.css_signature_len;
			    ...
			    struct icp_qat_fw_auth_desc *desc)
{
	...
	if (IS_QAT_GEN3(handle->pci_dev->device)) {
		fcu_ctl_csr = FCU_CONTROL_GEN3;
		fcu_sts_csr = FCU_STATUS_GEN3;
		fcu_dram_hi_csr = FCU_DRAM_ADDR_HI_GEN3;
		fcu_dram_lo_csr = FCU_DRAM_ADDR_LO_GEN3;
	} else {
	...
	qat_set_fcu_csr(handle, fcu_dram_hi_csr, (bus_addr >> BITS_IN_DWORD));
	qat_set_fcu_csr(handle, fcu_dram_lo_csr, bus_addr);
	qat_set_fcu_csr(handle, fcu_ctl_csr, FCU_CTRL_CMD_AUTH);

	do {
		msleep(FW_AUTH_WAIT_PERIOD);
		fcu_sts = qat_get_fcu_csr(handle, fcu_sts_csr);
...
auth_fail:
	...
	if (IS_QAT_GEN3(handle->pci_dev->device)) {
		mem_cfg_err = (qat_get_fcu_csr(handle, FCU_STATUS1_GEN3) &
				MEM_CFG_ERR_BIT);
		if (mem_cfg_err)
			       ...
			       struct icp_firml_dram_desc *dram_desc)
{
	...
	memzero_explicit(dram_desc, sizeof(*dram_desc));
}
				...
				struct icp_qat_fw_auth_desc **desc)
{
	...
	if (size <= css_fw_size.ae_img_offset) {
		pr_err("QAT: error, input image size too small %d\n", size);
		return -EINVAL;
	}

	if (size > (css_fw_size.ae_img_offset + ICP_QAT_CSS_MAX_IMAGE_LEN)) {
		pr_err("QAT: error, input image size overflow %d\n", size);
		return -EINVAL;
	}
	length = (css_hdr->fw_type == CSS_AE_FIRMWARE) ?
		 css_fw_size.css_ae_simg_len + simg_offset :
		 size + css_fw_size.css_fwsk_pad_len + simg_offset;
	...
	/* padding */
	memset((void *)(uintptr_t)(virt_addr +
	       css_fw_size.css_fwsk_modulus_len),
	       0, css_fw_size.css_fwsk_pad_len);

	/* exponent */
	memcpy((void *)(uintptr_t)(virt_addr +
	       css_fw_size.css_fwsk_modulus_len +
	       css_fw_size.css_fwsk_pad_len),
	       (void *)(image + sizeof(*css_hdr) +
			       css_fw_size.css_fwsk_modulus_len),
	       sizeof(unsigned int));
	...
	bus_addr = ADD_ADDR(auth_desc->fwsk_pub_high,
			    ...
			    css_fw_size.css_fwsk_pub_len;
	virt_addr = virt_addr + css_fw_size.css_fwsk_pub_len;
	...
	bus_addr = ADD_ADDR(auth_desc->signature_high,
			    ...
			    css_fw_size.css_signature_len;
	virt_addr += css_fw_size.css_signature_len;

	auth_desc->img_high = (unsigned int)(bus_addr >> BITS_IN_DWORD);
	auth_desc->img_low = (unsigned int)bus_addr;
	auth_desc->img_len = size - css_fw_size.ae_img_offset;
	memcpy((void *)(uintptr_t)virt_addr,
	       (void *)(image + css_fw_size.ae_img_offset),
	       auth_desc->img_len);
	virt_addr = virt_base;
	/* AE firmware */
			    ...
			    struct icp_qat_fw_auth_desc *desc)
{
	...
	for_each_set_bit(i, &ae_mask, handle->hal_handle->ae_max_num) {
		...
		qat_set_fcu_csr(handle, fcu_ctl_csr,
		...
		do {
			...
			if ((fcu_sts & FCU_AUTH_STS_MASK) ==
			    FCU_STS_LOAD_DONE) {
				loaded_aes =
					IS_QAT_GEN3(handle->pci_dev->device) ?
					qat_get_fcu_csr(handle,
							FCU_AE_LOADED_GEN3) :
		       ...
		       void *addr_ptr, int mem_size)
{
	...
	if (handle->fw_auth) {
		status = qat_uclo_map_auth_fw(handle, addr_ptr, mem_size,
					      &desc);
		if (!status)
			status = qat_uclo_auth_fw(handle, desc);
		qat_uclo_ummap_auth_fw(handle, &desc);
	} else {
		status = qat_uclo_wr_sram_by_words(handle, 0, addr_ptr,
						   mem_size);
	}
	return status;
}
EXPORT_SYMBOL_GPL(qat_uclo_wr_mimage);
				     ...
				     u32 mof_size)
{
	...
	min_ver_offset = mof_size - offsetof(struct icp_qat_mof_file_hdr,
						min_ver);
	checksum = qat_uclo_calc_str_checksum((char *)&mof_ptr->min_ver,
					       min_ver_offset);
...
{
	if ((strncmp((char *)(obj_chunkhdr->chunkId),
			ICP_QAT_UOF_IMAG, ICP_QAT_MOF_OBJ_CHUNKID_LEN)) == 0) {
		mobj_hdr->obj_buf = (char *)((unsigned long)obj_chunkhdr->offset
				     + mobj_handle->uobjs_hdr);
	} else if ((strncmp((char *)(obj_chunkhdr->chunkId),
			ICP_QAT_SUOF_IMAG, ICP_QAT_MOF_OBJ_CHUNKID_LEN)) == 0) {
		mobj_hdr->obj_buf = (char *)((unsigned long)obj_chunkhdr->offset
	...
	mobj_hdr->obj_name = (char *)(obj_chunkhdr->name +
				mobj_handle->sym_str);
	return 0;
}
...
{
	struct icp_qat_mof_objhdr *mof_objHdr;
	...
	mof_objHdr = (struct icp_qat_mof_objhdr *)
			kzalloc((uobj_chunk_num + sobj_chunk_num) *
			sizeof(*mof_objHdr), GFP_KERNEL);
	if (!mof_objHdr) {
		pr_err("QAT: memory allocation fail\n");
		return -ENOMEM;
	}

	mobj_handle->obj_table.obj_hdr = mof_objHdr;
	validChunks = &mobj_handle->obj_table.num_objs;
	...
	if ((uobj_chunk_num + sobj_chunk_num) != *validChunks) {
...
static void qat_uclo_map_mof_chunk(struct icp_qat_mof_handle *mobj_handle,
				  struct icp_qat_mof_chunkhdr *mof_chunkhdr)
{
	if (!strncmp(mof_chunkhdr->chunkId, ICP_QAT_MOF_SYM_OBJS,
			ICP_QAT_MOF_OBJ_ID_LEN))
		qat_uclo_map_mof_symobjs(mobj_handle, mof_chunkhdr);
	else if (!strncmp(mof_chunkhdr->chunkId, ICP_QAT_UOF_OBJS,
			     ICP_QAT_MOF_OBJ_ID_LEN))
		mobj_handle->uobjs_hdr = mobj_handle->mof_buf +
				(unsigned long)mof_chunkhdr->offset;
	else if (!strncmp(mof_chunkhdr->chunkId, ICP_QAT_SUOF_OBJS,
			     ICP_QAT_MOF_OBJ_ID_LEN))
				...
				unsigned int *obj_size)
{
	...
	if (!mobj_handle) {
		pr_err("QAT: memory allocation failure\n");
		return -ENOMEM;
	}
	...
	if (ret)
		return ret;
	mof_chunkhdr = (struct icp_qat_mof_chunkhdr *)
			((uintptr_t)mof_ptr + sizeof(*mof_ptr));
	...
	ret = qat_uclo_seek_obj_inside_mof(mobj_handle, obj_name,
					   obj_ptr, obj_size);
	if (ret)
		return ret;
	return 0;
...
EXPORT_SYMBOL_GPL(qat_uclo_map_obj);
...
EXPORT_SYMBOL_GPL(qat_uclo_del_uof_obj);
...
EXPORT_SYMBOL_GPL(qat_uclo_wr_all_uimage);
...
EXPORT_SYMBOL_GPL(qat_uclo_set_cfg_ae_mask);
