...
/* Copyright(c) 2014 - 2017, 2019 - 2021, 2023 Intel Corporation */
			       ...
			       int chk_inactive)
{
	...
	unsigned int csr = (1U << ACS_ABO_BITPOS);
	...
	while ((int)cycles > elapsed_cycles && times--) {
		...
		if (elapsed_cycles >= 8 && !(csr & (1U << ACS_ABO_BITPOS)))
			return 0;
	}
	if (times < 0) {
...
#define CLR_BIT(wrd, bit) ((wrd) & ~(1U << (bit)))
#define SET_BIT(wrd, bit) ((wrd) | 1U << (bit))
			     ...
			     unsigned char mode)
{
	...
	new_csr = (mode) ?
		  SET_BIT(csr, MMC_SHARE_CS_BITPOS) :
...
static unsigned int
qat_get_glb_csr(struct icp_qat_fw_loader_handle *handle,
		unsigned int csr)
{
	unsigned int val = 0;

	if (IS_QAT_GEN3(handle->pci_dev->device))
		val = GET_CAP_CSR(handle, csr);
	else
		val = GET_CAP_CSR(handle, csr + GLOBAL_CSR);
	return val;
}

static void
qat_set_glb_csr(struct icp_qat_fw_loader_handle *handle,
		unsigned int csr,
		unsigned int val)
{
	if (IS_QAT_GEN3(handle->pci_dev->device))
...
unsigned int
qat_get_fcu_csr(struct icp_qat_fw_loader_handle *handle,
		unsigned int csr)
{
	unsigned int val = 0;

	if (IS_QAT_GEN3(handle->pci_dev->device))
		val = GET_CAP_CSR(handle, csr + FCU_OFFSET_C4XXX);
	else
		val = GET_CAP_CSR(handle, csr + FCU_OFFSET);
	return val;
}

void
qat_set_fcu_csr(struct icp_qat_fw_loader_handle *handle,
		unsigned int csr,
		unsigned int val)
{
	if (IS_QAT_GEN3(handle->pci_dev->device))
...
void qat_hal_reset(struct icp_qat_fw_loader_handle *handle)
{
	...
	for (i = 0; i < cpp_num; i++) {
		if (i == 0) {
			...
			valid_ae_mask = (handle->hal_handle->ae_mask >>
					AES_PER_CPP) & 0xFFFF;
			valid_slice_mask = (handle->hal_handle->slice_mask >>
					   SLICES_PER_CPP) & 0x3F;
		}

		ae_reset_val[i] = qat_get_glb_csr(handle, ae_reset_csr[i]);
		ae_reset_val[i] |= valid_ae_mask << RST_CSR_AE_LSB;
		ae_reset_val[i] |= valid_slice_mask << RST_CSR_QAT_LSB;
		qat_set_glb_csr(handle, ae_reset_csr[i], ae_reset_val[i]);
	}
}
...
static void qat_hal_reset_timestamp(struct icp_qat_fw_loader_handle *handle)
{
	...
	misc_ctl_csr = (IS_QAT_GEN3(handle->pci_dev->device)) ?
			MISC_CONTROL_C4XXX : MISC_CONTROL;
	/* stop the timestamp timers */
	misc_ctl = qat_get_glb_csr(handle, misc_ctl_csr);
	if (misc_ctl & MC_TIMESTAMP_ENABLE)
		qat_set_glb_csr(handle, misc_ctl_csr, misc_ctl &
			    (~MC_TIMESTAMP_ENABLE));
	...
	qat_set_glb_csr(handle, misc_ctl_csr, misc_ctl | MC_TIMESTAMP_ENABLE);
}
...
int qat_hal_clr_reset(struct icp_qat_fw_loader_handle *handle)
{
	...
	for (i = 0; i < cpp_num; i++) {
		...
		ae_reset_val[i] &= ~(valid_slice_mask << RST_CSR_QAT_LSB);
		do {
			qat_set_glb_csr(handle, ae_reset_csr[i],
					ae_reset_val[i]);
			if (!(times--))
				goto out_err;
			ae_reset_val[i] = qat_get_glb_csr(handle,
							  ae_reset_csr[i]);
		} while ((valid_ae_mask |
			 (valid_slice_mask << RST_CSR_QAT_LSB)) &
			 ae_reset_val[i]);
		/* enable clock */
		clk_val[i] = qat_get_glb_csr(handle, clk_csr[i]);
		clk_val[i] |= valid_ae_mask << 0;
		clk_val[i] |= valid_slice_mask << 20;
		qat_set_glb_csr(handle, clk_csr[i], clk_val[i]);
	}
				...
				unsigned int words_num, u64 *uword)
{
	...
	even_uwrods = kcalloc(UWORD_IMG_SIZE, sizeof(*uword), GFP_KERNEL);
	if (!even_uwrods)
		goto even_alloc_err;

	odd_uwords = kcalloc(UWORD_IMG_SIZE, sizeof(*uword), GFP_KERNEL);
	if (!odd_uwords)
		goto odd_alloc_err;
	...
	if (even_cpy_cnt > 0 && even_cpy_cnt < UWORD_IMG_SIZE)
		qat_hal_wr_uwords(handle, even_ae, (uaddr + 1) / 2,
				  even_cpy_cnt, even_uwrods);
	else
		pr_err("QAT: array index out of bounds error\n");

	if (odd_cpy_cnt > 0 && odd_cpy_cnt < UWORD_IMG_SIZE)
		qat_hal_wr_uwords(handle, odd_ae, uaddr / 2,
				  odd_cpy_cnt, odd_uwords);
	else
		pr_err("QAT: array index out of bounds error\n");
...
static int qat_hal_check_imr(struct icp_qat_fw_loader_handle *handle)
{
	u8 reg_val = 0;

	if (handle->pci_dev->device != ADF_C3XXX_PCI_DEVICE_ID &&
	    handle->pci_dev->device != ADF_200XX_PCI_DEVICE_ID)
		return 0;

	pci_read_config_byte(handle->pci_dev, PCI_COMMAND, &reg_val);
	/*
	 * PCI command register memory bit and rambaseaddr_lo address
	 * are checked to confirm IMR2 is enabled in BIOS settings
	 */
	if ((reg_val & PCI_COMMAND_MEMORY) &&
	    qat_get_fcu_csr(handle, FCU_RAMBASE_ADDR_LO))
		return 0;
...
static void
qat_get_csr_offset(unsigned short device_id, unsigned int *cap_offset,
		   unsigned int *ae_offset, unsigned int *ep_offset)
{
	if (IS_QAT_GEN3(device_id)) {
		*cap_offset = ICP_QAT_CAP_OFFSET_C4XXX;
		*ae_offset = ICP_QAT_AE_OFFSET_C4XXX;
		*ep_offset = ICP_QAT_EP_OFFSET_C4XXX;
		return;
	}
	*cap_offset = ICP_QAT_CAP_OFFSET;
	*ae_offset = ICP_QAT_AE_OFFSET;
	*ep_offset = ICP_QAT_EP_OFFSET;
}

int qat_hal_init(struct adf_accel_dev *accel_dev)
{
	unsigned char ae;
	unsigned int cap_offset = 0;
	unsigned int ae_offset = 0;
	unsigned int ep_offset = 0;
	...
	if (handle->pci_dev->device == ADF_DH895XCC_PCI_DEVICE_ID ||
		...
		if (IS_QAT_GEN3(handle->pci_dev->device))
			sram_offset = 0x400000 +
				      accel_dev->aram_info->mmp_region_offset;
		handle->hal_sram_addr_v =
			(void __iomem *)((uintptr_t)sram_bar->virt_addr +
				sram_offset);
		handle->hal_sram_size = sram_bar->size;
	}
	qat_get_csr_offset(handle->pci_dev->device, &cap_offset,
			   &ae_offset, &ep_offset);
	handle->hal_cap_g_ctl_csr_addr_v =
		(void __iomem *)((uintptr_t)misc_bar->virt_addr + cap_offset);
	handle->hal_cap_ae_xfer_csr_addr_v =
		(void __iomem *)((uintptr_t)misc_bar->virt_addr + ae_offset);
	handle->hal_ep_csr_addr_v =
		(void __iomem *)((uintptr_t)misc_bar->virt_addr + ep_offset);
	...
	if (handle->fw_auth && qat_hal_check_imr(handle)) {
		dev_err(&GET_DEV(accel_dev), "IMR2 not enabled in BIOS\n");
		goto out_hal_handle;
	}
		   ...
		   unsigned int ctx_mask)
{
	...
	if (handle->fw_auth) {
		...
		qat_set_fcu_csr(handle, fcu_ctl_csr, FCU_CTRL_CMD_START);
		do {
				   ...
				   unsigned int *endpc)
{
	...
	qat_hal_rd_indr_csr(handle, ae, ctx, INDIRECT_LM_ADDR_1_BYTE_INDEX,
			    &ind_lm_addr_byte1);
	if (IS_QAT_GEN3(handle->pci_dev->device)) {
		qat_hal_rd_indr_csr(handle, ae, ctx,
				    LM_ADDR_2_INDIRECT, &ind_lm_addr2);
		qat_hal_rd_indr_csr(handle, ae, ctx,
				    LM_ADDR_3_INDIRECT, &ind_lm_addr3);
		qat_hal_rd_indr_csr(handle, ae, ctx,
