	...
	if (!accel_dev) {
		...
		list_for_each_entry(tmp_dev, adf_devmgr_get_head(), list) {
			if (adf_dev_started(tmp_dev) &&
			    !list_empty(&tmp_dev->crypto_list)) {
				accel_dev = tmp_dev;
				pr_info("QAT: Device found on remote node %d different from application node %d\n",
...
static int qat_crypto_asym_configure(struct adf_accel_dev *accel_dev,
...
static int qat_crypto_sym_configure(struct adf_accel_dev *accel_dev,
...
static int qat_crypto_inst_configure(struct adf_accel_dev *accel_dev,
				     u32 i)
{
	char key[ADF_CFG_MAX_KEY_LEN_IN_BYTES];
	unsigned long val;
	...
	snprintf(key, sizeof(key), ADF_CY "%d" ADF_ETRMGR_CORE_AFFINITY, i);
	if (adf_cfg_add_key_value_param(accel_dev, ADF_KERNEL_SEC,
					key, (void *)&val, ADF_DEC))
		goto err;
	...
	return 0;
err:
	dev_err(&GET_DEV(accel_dev), "Failed to configure instance\n");
	return -EINVAL;
}
...
{
	unsigned long val = 0;
	unsigned long ring_pair_index = 0;
	u8 serv_type = 0;
	struct adf_hw_device_data *hw_data = GET_HW_DATA(accel_dev);
	u16 ring_to_svc_map = hw_data->ring_to_svc_map;
	unsigned long  num_req_rings = hw_data->num_rings_per_bank / 2;
	unsigned long  num_rings_per_srv =
		num_req_rings / ADF_CFG_NUM_SERVICES;
	unsigned long  tx_ring = 0;
	...
	if (qat_crypto_inst_configure(accel_dev, val))
		goto err;
	...
	for (ring_pair_index = 0;
		...
		switch (serv_type) {
		case ASYM:
			if (qat_crypto_asym_configure(accel_dev, tx_ring, 0))
				goto err;
			break;
		case SYM:
			if (qat_crypto_sym_configure(accel_dev, tx_ring, 0))
				goto err;
			break;
		case CRYPTO:
			if (qat_crypto_asym_configure(accel_dev, tx_ring, 0))
...
int qat_crypto_dev_config(struct adf_accel_dev *accel_dev)
{
	...
	for (i = 0; i < instances; i++) {
		if (qat_crypto_inst_configure(accel_dev, i))
			goto err;
	}
	if (capabilities & ADF_ACCEL_CAPABILITIES_CRYPTO_ASYMMETRIC) {
		for (i = 0; i < instances; i++) {
			if (qat_crypto_asym_configure(accel_dev, val, i))
				goto err;
		}
	}
	...
	if (capabilities & ADF_ACCEL_CAPABILITIES_CRYPTO_SYMMETRIC) {
		for (i = 0; i < instances; i++) {
			if (qat_crypto_sym_configure(accel_dev, val, i))
				goto err;
		}
	}
					...
					int i)
{
	...
	if (adf_cfg_get_param_value(accel_dev, SEC, key, val))
		goto err;
	ret_val = kstrtoul(val, 10, &num_msg_asym);
	if (ret_val)
		goto err;
				       ...
				       int i)
{
	...
	ret_val = kstrtoul(val, 10, &num_msg_sym);
	if (ret_val)
		goto err;
...
static int qat_crypto_create_instances(struct adf_accel_dev *accel_dev)
{
	...
	ret_val = kstrtoul(val, 0, &num_inst);
	if (ret_val)
		return ret_val;
	...
	for (i = 0; i < num_inst; i++) {
		...
		if (hw_data->accel_capabilities_mask &
				ADF_ACCEL_CAPABILITIES_CRYPTO_ASYMMETRIC) {
			ret_val = qat_crypto_create_asym_rings(
					accel_dev, inst, i);
			if (ret_val)
				goto err;
		}
		if (hw_data->accel_capabilities_mask &
				ADF_ACCEL_CAPABILITIES_CRYPTO_SYMMETRIC) {
			ret_val = qat_crypto_create_sym_rings(
					accel_dev, inst, i);
			if (ret_val)
				goto err;
		}
...
static int qat_crypto_init(struct adf_accel_dev *accel_dev)
{
	...
	ret_val = qat_crypto_create_instances(accel_dev);
	return ret_val;
}
