				  ...
				  struct adf_accel_dev *accel_dev)
{
	...
	if (bundle->polling_mode != inst->polling_mode)
		return false;
	...
	if (accel_dev->is_user_bundle_dist_needed &&
	    inst->stype != COMP && device->bundles_free)
		return false;
					...
					const char *process_name)
{
	...
	if (adf_cfg_can_be_shared(bundle, process_name, inst,
				  device, accel_dev)) {
			...
			struct adf_accel_dev *accel_dev)
{
	...
	ret = adf_cfg_init_ring2serv_mapping(accel_dev, bundle);
	if (ret)
		goto failed;
...
static int adf_cfg_assign_serv_to_rings(struct adf_cfg_bundle *bundle,
					u16 ring_to_svc_map)
{
	int ring_pair_index = 0;
	int ring_index = 0;
					  ...
					  struct adf_cfg_bundle *bundle)
{
	...
	if (adf_cfg_assign_serv_to_rings(bundle, hw_data->ring_to_svc_map))
		goto failed;
