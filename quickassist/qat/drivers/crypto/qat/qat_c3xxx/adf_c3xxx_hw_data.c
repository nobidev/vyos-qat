...
static u32 get_num_accels(struct adf_hw_device_data *self)
{
	...
	for_each_set_bit(i, &self->accel_mask, ADF_C3XXX_MAX_ACCELERATORS)
		ctr++;
	return ctr;
}
...
static u32 get_num_aes(struct adf_hw_device_data *self)
{
	...
	for_each_set_bit(i, &self->ae_mask, ADF_C3XXX_MAX_ACCELENGINES)
		ctr++;
	return ctr;
}
				      ...
				      u32 vf_int_mask_sets[ADF_MAX_VF2PF_SET])
{
	...
	sources = ADF_C3XXX_ERR_REG_VF2PF(errsou3);

	/* Get the already disabled interrupts */
	errmsk3 = ADF_CSR_RD(pmisc_addr, ADF_ERRMSK3);
	non_vf2pf_errmsk3 = errmsk3 & ADF_C3XXX_ERRMSK3_NON_VF2PF;
	disabled = ADF_C3XXX_ERR_REG_VF2PF(errmsk3);
	 ...
	 */
	errmsk3 |= ADF_C3XXX_ERRMSK3_VF2PF(ADF_VF2PF_INT_MASK);
	ADF_CSR_WR(pmisc_addr, ADF_ERRMSK3, errmsk3);

	errmsk3 =
		non_vf2pf_errmsk3 | ADF_C3XXX_ERRMSK3_VF2PF(sources | disabled);
	ADF_CSR_WR(pmisc_addr, ADF_ERRMSK3, errmsk3);
}
				    ...
				    u32 vf_mask, u8 vf2pf_set)
{
	...
	if (vf_mask & 0xFFFF)
		adf_csr_fetch_and_and(pmisc_addr,
				      ADF_ERRMSK3,
				      ~ADF_C3XXX_ERRMSK3_VF2PF(vf_mask));
}
				    ...
				    u32 const **arb_map_config)
{
	...
	for_each_set_bit(i, &hw_device->ae_mask, ADF_C3XXX_MAX_ACCELENGINES)
...
{
	ADF_CSR_WR(csr, ADF_ERRMSK0, ADF_C3XXX_ERRMSK0_CERR); /* ME0-ME3 */
	ADF_CSR_WR(csr, ADF_ERRMSK1, ADF_C3XXX_ERRMSK1_CERR); /* ME4-ME5 */
	ADF_CSR_WR(csr, ADF_ERRMSK5, ADF_C3XXX_ERRMSK5_CERR); /* SSM2 */

	/* Reset everything except VFtoPF1_16. */
	adf_csr_fetch_and_and(csr, ADF_ERRMSK3, ADF_C3XXX_VF2PF1_16);

	/* RI CPP bus interface error detection and reporting. */
	ADF_CSR_WR(csr, ADF_C3XXX_RICPPINTCTL, ADF_C3XXX_RICPP_EN);

	/* TI CPP bus interface error detection and reporting. */
	ADF_CSR_WR(csr, ADF_C3XXX_TICPPINTCTL, ADF_C3XXX_TICPP_EN);

	/* Enable CFC Error interrupts and logging. */
	ADF_CSR_WR(csr, ADF_C3XXX_CPP_CFC_ERR_CTRL, ADF_C3XXX_CPP_CFC_UE);
}
...
static void adf_disable_error_interrupts(struct adf_accel_dev *accel_dev)
{
	...
	/* ME0-ME3 */
	ADF_CSR_WR(csr, ADF_ERRMSK0, ADF_C3XXX_ERRMSK0_UERR |
		   ADF_C3XXX_ERRMSK0_CERR);
	/* ME4-ME5 */
	ADF_CSR_WR(csr, ADF_ERRMSK1, ADF_C3XXX_ERRMSK1_UERR |
		   ADF_C3XXX_ERRMSK1_CERR);
	/* CPP Push Pull, RI, TI, SSM0-SSM1, CFC */
	ADF_CSR_WR(csr, ADF_ERRMSK3, ADF_C3XXX_ERRMSK3_UERR);
	/* SSM2 */
	ADF_CSR_WR(csr, ADF_ERRMSK5, ADF_C3XXX_ERRMSK5_UERR);
}
...
static int adf_check_uncorrectable_error(struct adf_accel_dev *accel_dev)
{
	...
	u32 errsou0 = ADF_CSR_RD(csr, ADF_ERRSOU0) & ADF_C3XXX_ERRMSK0_UERR;
	u32 errsou1 = ADF_CSR_RD(csr, ADF_ERRSOU1) & ADF_C3XXX_ERRMSK1_UERR;
	u32 errsou3 = ADF_CSR_RD(csr, ADF_ERRSOU3) & ADF_C3XXX_ERRMSK3_UERR;
	u32 errsou5 = ADF_CSR_RD(csr, ADF_ERRSOU5) & ADF_C3XXX_ERRMSK5_UERR;
					    ...
					    struct adf_hw_device_data *hw_data)
{
	...
	for_each_set_bit(dev, &hw_data->accel_mask,
		...
		/* Set power-up */
		adf_csr_fetch_and_and(csr, ADF_C3XXX_SLICEPWRDOWN(dev),
				      ~ADF_C3XXX_MMP_PWR_UP_MSK);
		...
		adf_csr_fetch_and_or(csr, ADF_C3XXX_SLICEPWRDOWN(dev),
				     ADF_C3XXX_MMP_PWR_UP_MSK);

		/* Disabling correctable error interrupts. */
		ADF_CSR_WR(csr, ADF_C3XXX_INTMASKSSM(dev),
			   ADF_C3XXX_INTMASKSSM_UERR);
	}
}
...
static void adf_enable_error_correction(struct adf_accel_dev *accel_dev)
{
	...
	for_each_set_bit(i, &hw_device->ae_mask, ADF_C3XXX_MAX_ACCELENGINES) {
		adf_csr_fetch_and_or(csr, ADF_C3XXX_AE_CTX_ENABLES(i),
				     ADF_C3XXX_ENABLE_AE_ECC_ERR);
		adf_csr_fetch_and_or(csr, ADF_C3XXX_AE_MISC_CONTROL(i),
...
static u32 c3xxx_get_hw_cap(struct adf_accel_dev *accel_dev)
{
	struct pci_dev *pdev = accel_dev->accel_pci_dev.pci_dev;
	u32 legfuses;
	u32 capabilities;
...
static int get_sla_units(struct adf_accel_dev *accel_dev, u32 **sla_au)
{
	...
	if (i < ARRAY_SIZE(slau_cfg)) {
		if (clock_frequency <= ADF_C3XXX_MIN_AE_FREQ)
			*sla_au = slau_cfg[i].slau_supported[DEV_SKU_1];
		else if (clock_frequency > ADF_C3XXX_MIN_AE_FREQ &&
			 clock_frequency < ADF_C3XXX_MAX_AE_FREQ)
			*sla_au = slau_cfg[i].slau_supported[DEV_SKU_2];
		else
			*sla_au = slau_cfg[i].slau_supported[DEV_SKU_3];
	}
...
static void adf_dev_pre_reset_c3xxx(struct adf_accel_dev *accel_dev)
{
	adf_clear_uncorrectable_error_regs(accel_dev);
...
void adf_init_hw_data_c3xxx(struct adf_hw_device_data *hw_data)
{
	...
	hw_data->pre_reset = adf_dev_pre_reset_c3xxx;
