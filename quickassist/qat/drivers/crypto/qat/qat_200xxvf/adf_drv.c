...
static const struct pci_device_id adf_pci_tbl[] = {
	ADF_SYSTEM_DEVICE(ADF_200XXIOV_PCI_DEVICE_ID),
	{0,}
};
...
static void adf_cleanup_accel(struct adf_accel_dev *accel_dev)
{
	...
	if (accel_dev->hw_device) {
		...
		case ADF_200XXIOV_PCI_DEVICE_ID:
			adf_clean_hw_data_200xxiov(accel_dev->hw_device);
			break;
		default:
			break;
		}
...
static int adf_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	adf_init_hw_data_200xxiov(accel_dev->hw_device);
	pci_read_config_byte(pdev, PCI_REVISION_ID, &accel_pci_dev->revid);
	pci_read_config_dword(pdev, ADF_200XX_VFFUSECTL_OFFSET,
			      &hw_data->fuses);
