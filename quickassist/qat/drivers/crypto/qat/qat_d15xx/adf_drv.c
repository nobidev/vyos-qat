...
static const struct pci_device_id adf_pci_tbl[] = {
	ADF_SYSTEM_DEVICE(ADF_D15XX_PCI_DEVICE_ID),
	{0,}
};
...
static void adf_cleanup_accel(struct adf_accel_dev *accel_dev)
{
	...
	if (accel_dev->hw_device) {
		...
		case ADF_D15XX_PCI_DEVICE_ID:
			adf_clean_hw_data_d15xx(accel_dev->hw_device);
			break;
		default:
			break;
		}
...
static int adf_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	adf_init_hw_data_d15xx(accel_dev->hw_device);
...
MODULE_AUTHOR("Intel");
MODULE_FIRMWARE(ADF_D15XX_FW);
MODULE_FIRMWARE(ADF_D15XX_MMP);
