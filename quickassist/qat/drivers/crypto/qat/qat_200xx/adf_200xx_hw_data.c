...
static struct adf_hw_device_class adf_200xx_class = {
...
static u32 get_accel_mask(struct adf_accel_dev *accel_dev)
{
	...
	pci_read_config_dword(pdev, ADF_DEVICE_FUSECTL_OFFSET,
			      &fuse);
	pci_read_config_dword(pdev, ADF_200XX_SOFTSTRAP_CSR_OFFSET,
			      &straps);
...
static u32 get_num_accels(struct adf_hw_device_data *self)
{
	...
	for_each_set_bit(i, &self->accel_mask, ADF_200XX_MAX_ACCELERATORS)
		ctr++;
	return ctr;
}
...
static u32 get_num_aes(struct adf_hw_device_data *self)
{
	...
	for_each_set_bit(i, &self->ae_mask, ADF_200XX_MAX_ACCELENGINES)
		ctr++;
	return ctr;
}
				      ...
				      u32 vf_int_mask_sets[ADF_MAX_VF2PF_SET])
{
	...
	sources = ADF_200XX_ERR_REG_VF2PF(errsou3);

	/* Get the already disabled interrupts */
	errmsk3 = ADF_CSR_RD(pmisc_addr, ADF_ERRMSK3);
	non_vf2pf_errmsk3 = errmsk3 & ADF_200XX_ERRMSK3_NON_VF2PF;
	disabled = ADF_200XX_ERR_REG_VF2PF(errmsk3);
	 ...
	 */
	errmsk3 |= ADF_200XX_ERRMSK3_VF2PF(ADF_VF2PF_INT_MASK);
	ADF_CSR_WR(pmisc_addr, ADF_ERRMSK3, errmsk3);

	errmsk3 =
		non_vf2pf_errmsk3 | ADF_200XX_ERRMSK3_VF2PF(sources | disabled);
	ADF_CSR_WR(pmisc_addr, ADF_ERRMSK3, errmsk3);
}
				    ...
				    u32 vf_mask, u8 vf2pf_set)
{
	...
	if (vf_mask & 0xFFFF)
		adf_csr_fetch_and_and(pmisc_addr,
				      ADF_ERRMSK3,
				      ~ADF_200XX_ERRMSK3_VF2PF(vf_mask));
}
				    ...
				    u32 const **arb_map_config)
{
	...
	for_each_set_bit(i, &hw_device->ae_mask, ADF_200XX_MAX_ACCELENGINES)
...
static void adf_disable_error_interrupts(struct adf_accel_dev *accel_dev)
{
	struct adf_bar *misc_bar =
		&GET_BARS(accel_dev)[ADF_200XX_PMISC_BAR];
	void __iomem *csr = misc_bar->virt_addr;

	/* ME0-ME3 */
	ADF_CSR_WR(csr, ADF_ERRMSK0, ADF_200XX_ERRMSK0_UERR |
		   ADF_200XX_ERRMSK0_CERR);
	/* ME4-ME5 */
	ADF_CSR_WR(csr, ADF_ERRMSK1, ADF_200XX_ERRMSK1_UERR |
...
static void adf_enable_error_correction(struct adf_accel_dev *accel_dev)
{
	...
	for_each_set_bit(i, &hw_device->ae_mask, ADF_200XX_MAX_ACCELENGINES) {
		adf_csr_fetch_and_or(csr, ADF_200XX_AE_CTX_ENABLES(i),
				     ADF_200XX_ENABLE_AE_ECC_ERR);
		adf_csr_fetch_and_or(csr, ADF_200XX_AE_MISC_CONTROL(i),
				     ADF_200XX_ENABLE_AE_ECC_PARITY_CORR);
	}
...
static void adf_enable_ints(struct adf_accel_dev *accel_dev)
{
	...
	ADF_CSR_WR(addr, ADF_200XX_SMIAPF0_MASK_OFFSET,
		   ADF_200XX_SMIA0_MASK);
	ADF_CSR_WR(addr, ADF_200XX_SMIAPF1_MASK_OFFSET,
		   ADF_200XX_SMIA1_MASK);
}
...
static u32 adf_200xx_get_hw_cap(struct adf_accel_dev *accel_dev)
{
	struct pci_dev *pdev = accel_dev->accel_pci_dev.pci_dev;
	u32 legfuses;
	u32 capabilities;
...
static int get_sla_units(struct adf_accel_dev *accel_dev, u32 **sla_au)
{
	...
	if (i < ARRAY_SIZE(slau_cfg)) {
		if (clock_frequency <= ADF_200XX_MIN_AE_FREQ)
			*sla_au = slau_cfg[i].slau_supported[DEV_SKU_1];
		else if (clock_frequency > ADF_200XX_MIN_AE_FREQ &&
			 clock_frequency < ADF_200XX_MAX_AE_FREQ)
...
static void adf_dev_pre_reset_200xx(struct adf_accel_dev *accel_dev)
{
...
{
	hw_data->dev_class = &adf_200xx_class;
	hw_data->instance_id = adf_200xx_class.instances++;
	...
	hw_data->pre_reset = adf_dev_pre_reset_200xx;
