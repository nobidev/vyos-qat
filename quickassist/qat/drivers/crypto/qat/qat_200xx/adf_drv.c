...
static const struct pci_device_id adf_pci_tbl[] = {
	ADF_SYSTEM_DEVICE(ADF_200XX_PCI_DEVICE_ID),
	{0,}
};
...
static void adf_cleanup_accel(struct adf_accel_dev *accel_dev)
{
	...
	if (accel_dev->hw_device) {
		...
		case ADF_200XX_PCI_DEVICE_ID:
			adf_clean_hw_data_200xx(accel_dev->hw_device);
			break;
		default:
			break;
		}
...
static int adf_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	adf_init_hw_data_200xx(accel_dev->hw_device);
	pci_read_config_byte(pdev, PCI_REVISION_ID, &accel_pci_dev->revid);
	pci_read_config_dword(pdev, ADF_DEVICE_FUSECTL_OFFSET,
			      &hw_data->fuses);

	/* Clear PPAERUCSEV value, Get PPAERUCM value and store */
	aer_offset = pci_find_ext_capability(pdev, PCI_EXT_CAP_ID_ERR);
	pci_write_config_dword(pdev, aer_offset + PCI_ERR_UNCOR_SEVER, 0);
	pci_read_config_dword(pdev, aer_offset + PCI_ERR_UNCOR_MASK, &reg_val);
	hw_data->aerucm_mask = reg_val;

	/* Clear PFIEERRUNCSTSR register bits if they are set */
	pci_read_config_dword(pdev, ADF_PFIEERRUNCSTSR, &reg_val);
	if (reg_val) {
		dev_warn(&pdev->dev,
			 "Clearing PFIEERRUNCSTSR, previous status : %0x\n",
			 reg_val);
		pci_write_config_dword(pdev, ADF_PFIEERRUNCSTSR, reg_val);
	}
	...
	if (!hw_data->accel_mask || (~hw_data->ae_mask & 0x01)) {
...
MODULE_AUTHOR("Intel");
MODULE_FIRMWARE(ADF_200XX_FW);
MODULE_FIRMWARE(ADF_200XX_MMP);
